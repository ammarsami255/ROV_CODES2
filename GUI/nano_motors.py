...
def send_command(command):
    if arduino:
        ...
        time.sleep(0.05)
        try:
            response = arduino.readline().decode('utf-8').strip()  # محاولة فك التشفير بـ UTF-8
...
def test_all_motors():
    for i in range(1, 7):
        control_motor(i, 128)  # تعيين السرعة لجميع المحركات
        time.sleep(1)
...
def key_press(event):
    ...
    if event.keysym == "Up":
        ...
        control_motor(int(event.char), 128)  # التحكم بالسرعة للمحرك
    elif event.char == "p":
        read_pressure()
    elif event.char == "l":
        toggle_led()
...
def open_camera():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            control_motor(i, 128)  # تعيين السرعة عند الضغط على الأزرار
        elif key == ord('p'):
            send_command("READ_MS5540C")
        elif key == ord('l'):
            toggle_led()
